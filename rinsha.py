...
def takeBackup (ssh, jobs, userDirName):
	...
	try:
		...
		os.system (" sshpass -p \"rinsha@123\" rsync --remove-source-files --progress -avz {}@10.24.6.200:{}/dump_nvt.lammpstrj {}/dump_new.lammpstrj".format (userDirName, jobs, targetLocation))
	...
	for jobs in finishedJobs:
		...
		for login in loginList:
			if (login in jobs):
				while (success == 0):
					try:
						currentOccupiedSpace = checkOccupiedSpace (ssh, "mm18d016")
						...
						if (currentOccupiedSpace > 40.0):
							...
							takeBackup (ssh, jobs, "mm18d016")
						ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command("cd {}; qsub rpSimulations.cmd".format (jobs))
						success = 1
					except:
						success = 0
						ssh.close ()
						...
						try:
							ssh.connect(ip_hpce, username = credentials ["rinsha"][0], password = credentials ["rinsha"][1], look_for_keys = False)
						except Exception as e:
							pass
